...
module EmpexBlockchain
  class RetrieveEmployeeExperienceRequest
    include RequestHandler

    attr_accessor :employee_experience, :employee

    def initialize(employee_experience, employee)
      @employee_experience = employee_experience
      @employee = employee
    end

    def url
      'http://localhost:8081/web3/retrieveEmployeeExperience'
    end
    ...
    def create_post(response_body)
      Post.create_record(
        user_id: user_id,
        request_type: 'Retrieve Employee Experience',
        request: message,
